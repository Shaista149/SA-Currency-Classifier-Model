"""
coin_augment_extra.py - Extra augmentation for SA coin dataset

Writes new augmented images to a SEPARATE folder (coins/augmented_extra/)
and creates a NEW CSV (coin_labels_extra.csv).

Nothing in your existing augmented/ folder or coin_labels.csv is touched.
...
Folder structure expected:
    Currency_Classifier/data/
        ...
        coin_labels.csv             <- existing CSV (NOT touched)
        coins/
        ...
        coin_labels_extra.csv       <- new CSV written here
"""
...
OUTPUT_DIR = os.path.join("coins", "augmented_extra")
CSV_PATH   = "coin_labels_extra.csv"
BG_DIR     = "background_images"

def perspective_warp(image, strength=0.15):
    """Simulate the coin being photographed at a slight angle."""
    h, w = image.shape[:2]
...
def paste_on_background(coin_img, bg_img):
    """
    Mask out the white background of the flat coin and paste it
    ...
    mask_coin = cv2.bitwise_not(mask_bg)
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5))
    mask_coin = cv2.morphologyEx(mask_coin, cv2.MORPH_CLOSE, kernel)
    mask_bg = cv2.bitwise_not(mask_coin)
    coin_part = cv2.bitwise_and(coin_img, coin_img, mask=mask_coin)
    bg_part = cv2.bitwise_and(bg, bg, mask=mask_bg)
    return cv2.add(coin_part, bg_part)
...
# Write new CSV (does NOT touch existing coin_labels.csv)
...
print(f"  2. Copy the class folders into 'coins/augmented/'")
print(f"  3. Append '{CSV_PATH}' rows to 'coin_labels.csv'")
